...
def find_prompts(cand_prompts, loss_table, eval_template,
                 demos_template,
                 prompt_gen_data,
                 eval_data,
                 conf,
    ...
    if cand_prompts is None:
        ...
        prompts = cand_prompts
    
    #print('Evaluating prompts...')

    if eval_data is None:
        print('we have no eval data at the moment (highly likely that we are using e-APE), so return entire generated prompts')
        #res = (prompts, None)
        res = prompts
    else:
