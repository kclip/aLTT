...
from huggingface_completion import HuggingFaceCompletion 
...
class GPT_Forward(LLM):
    ...
    def __init__(self, config, needs_confirmation=False, disable_tqdm=True):
        ...
        self.HFCompletion = HuggingFaceCompletion()
    ...
    def generate_text(self, prompt, n):
        ...
        for prompt_batch in tqdm(prompt_batches, disable=False):
            #print('prompt_batch', len(prompt_batch))
    ...
    def __generate_text(self, prompt, n):
        ...
        response = None
        response = self.HFCompletion.create(**config, prompt=prompt)
        while response is None:
            try:
                if IF_API:
                    ...
                    #print('config: ', config)
                    response = self.HFCompletion.create(**config, prompt=prompt)
                    #response = self.HFCompletion.create(prompt, model=config['model'], temperature=config['temperature'], max_tokens=config['max_tokens'],  top_p=config['top_p'], frequency_penalty=config['frequency_penalty'], n=config['n'])
                ...
                time.sleep(5)
        #print('----suffix-----', suffix)
        #print('before ------', [response['choices'][i]['text'] for i in range(len(response['choices']))])
        return [response['choices'][i]['text'] for i in range(len(response['choices']))]
        # this ensures extraction of the only generated part
        #tmp = [response['choices'][i]['text'].replace(suffix, '') for i in range(len(response['choices']))]
        #return [response['choices'][i]['text'].replace(suffix, '') for i in range(len(response['choices']))]
        ...
        while response is None:
            try:
                if IF_API:
                    ...
                    response = self.HFCompletion.create(**config, prompt=text)
            except Exception as e:
                print(e)
                print('Retrying...')
                time.sleep(5)
...
class GPT_Insert(LLM):
    ...
    def __generate_text(self, prompt, n):
        ...
        while response is None:
            try:
                if IF_API:
                    ...
                    response = self.HFCompletion.create(**config, prompt=prefix, suffix=suffix)
            except Exception as e:
                print(e)
                print('Retrying...')
                time.sleep(5)
        ...
        print('------------------------------------------------------------------')
        print('suffix:', suffix, 'output', response['choices'][0])
        print('------------------------------------------------------------------')
