...
def plot(test_dict, task, risk_control_mode):
    ...
    for mode in test_dict.keys():
        if mode == '': #mode == 'e_APE_epsilon_greedy_0.75' or mode == 'e_APE_epsilon_greedy_0.5':
            ...
            # axs.plot(get_stat_from_dict_list(test_dict, mode, 'cost')[0], get_stat_from_dict_list(test_dict, mode, 'min_len')[0], color=colors[ind_mode], marker=markers[ind_mode], markersize=0, linestyle=lines[ind_mode],label=mode_name)
            # axs.fill_between(get_stat_from_dict_list(test_dict, mode, 'cost')[0], get_stat_from_dict_list(test_dict, mode, 'min_len')[0]-get_stat_from_dict_list(test_dict, mode, 'min_len')[1], get_stat_from_dict_list(test_dict, mode, 'min_len')[0]+get_stat_from_dict_list(test_dict, mode, 'min_len')[1], alpha=0.2, facecolor=colors[ind_mode],linestyle=lines[ind_mode])
    ...
    #axs[1].legend()
    plt.tight_layout() 
    path = Path('./vis_results/ICML_LLM/fig1/main/' + str(risk_control_mode) +'/' + str(alpha)+ '/' + task + '.png')
...
if __name__ == '__main__':
    ...
    #alpha = 0.3
    delta = 0.1
    tasks = ['active_to_passive', 'diff', 'first_word_letter','larger_animal', 'letters_list',  'num_to_verbal', 'rhymes', 'sentiment', 'singular_to_plural', 'sum', 'taxonomy_animal']
    
    
    #task = 'word_in_context'
    save_dir_name = f'experiments/ICML_real_final/all_tasks/for_plotting/instruction_induction/{tasks[0]}/{args.risk_control_mode}/'
    ...
    for mode in test_dict.keys():
        ...
        for task in tasks:
            ...
            ind_task += 1
    plot(test_dict_per_task, task, args.risk_control_mode)
