...
def plot(test_dict, task, risk_control_mode):
    ...
    for mode in test_dict.keys():
        ...
        axs[3].plot(get_stat_from_dict_list(test_dict, mode, 'alpha')[0], get_stat_from_dict_list(test_dict, mode, 'min_len')[0], color=colors[ind_mode], marker=markers[ind_mode], markersize=0, linestyle=lines[ind_mode],label=mode_name)
        axs[3].fill_between(get_stat_from_dict_list(test_dict, mode, 'alpha')[0], get_stat_from_dict_list(test_dict, mode, 'min_len')[0]-get_stat_from_dict_list(test_dict, mode, 'min_len')[1], get_stat_from_dict_list(test_dict, mode, 'min_len')[0]+get_stat_from_dict_list(test_dict, mode, 'min_len')[1], alpha=0.2, facecolor=colors[ind_mode],linestyle=lines[ind_mode])
...
if __name__ == '__main__':
    ...
    total_tasks = ['active_to_passive', 'diff', 'first_word_letter','larger_animal', 'letters_list',  'num_to_verbal', 'rhymes', 'sentiment', 'singular_to_plural', 'sum', 'taxonomy_animal']
    

    def max_len_entire_cand_prompts(task):
        total_prompts = np.load('./cache/' + task + '/total_cand_prompts.npy')
        # print(total_prompts)
        max_len = 0
    ...
    for task in total_tasks:
        tasks = [task]
        test_dict_per_alpha = {}
        alpha_list = [0.7, 0.6, 0.5, 0.4, 0.3, 0.2]#, 0.1, 0.05]
        ind_alpha = 0

        total_budget_list =[10,15,20,50,100,150,200,500,1000,1500,2000]
        ind_budget = 10
        ...
        for alpha in alpha_list:
            ...
            for mode in ['e_APE_epsilon_greedy_0.25','e_APE_epsilon_greedy_0.5','e_APE_epsilon_greedy_0.75','e_APE_epsilon_greedy_0.95','p_APE','n_e_APE']: #, 'standard_APE', 'bandit_APE','e_APE_epsilon_greedy_0.0','e_APE_epsilon_greedy_0.1', 'e_APE_epsilon_greedy_0.25','e_APE_epsilon_greedy_0.5','e_APE_epsilon_greedy_0.75','e_APE_epsilon_greedy_0.95','e_APE_LCB']:
                ...
                for task in tasks:
                    ...
                    test_dict_per_task[mode]['min_len'][ind_task*num_repetition:(ind_task+1)*num_repetition] = np.nan_to_num(test_dict[mode]['min_len'], nan=max_len_entire_cand_prompts(task))
                    ind_task += 1
            ...
            for mode in test_dict_per_task.keys():
                ...
                # print(tmp)
                test_dict_per_alpha[mode]['min_len'][:, ind_alpha] = test_dict_per_task[mode]['min_len'][:, ind_budget]
            ind_alpha += 1
