...
def get_stat_from_dict_list(test_dict, mode, name):
    ...
    # return np.nanmean(info, axis=0), 0*np.nanstd(info, axis=0)/np.sqrt(info.shape[0])
...
def plot(test_dict, task, risk_control_mode):
    ...
    # axs.set_xscale('log')
    axs.grid()
    axs.set_ylim(bottom=30.5, top=50)
    ...
    #axs[1].legend()
    plt.tight_layout() 
    path = Path('./vis_results/ICML_LLM/fig2/main/per_alpha_' + str(risk_control_mode) +'/' + str(alpha)+str(delta)+ task + '.png')
