...
class TimeFreqResourceAllocationV0(Env):
    ...
    def __init__(self, n_ues=32, n_prbs=25, buffer_max_size=32, p_max=23,p_min=-70,alpha=1., f_carrier_mhz=2655,
    ...
    def reset(self):
        ...
        self.qi = np.concatenate((
                                  np.repeat(np.array([[0, 0, 0, 1]]), self.K // 2, axis=0),
                                  np.repeat(np.array([[0, 1, 0, 0]]), self.K // 2, axis=0))
                                 )
        self.p = 0
        self.t = 0
        self.tti = 0
        ...
        self._update_state()
        self.tx_powers=self._recalculate_rf()
    ...
    def step(self, action):
        ...
        if np.sum(self.s[action, :]) > 0:  # If packets exist in UE's buffer
            ...
            while tx_data_bits > 0 and self.s[action, l_old] > 0:  # While there are packets & available capacity
                if tx_data_bits >= self.s[action, l_old]:  # Full packet transmission
                    ...
                    break
        tx_power_mW = np.zeros(self.K)
        tx_bits = np.zeros(self.K)
        tx_bits[action]=bits-np.sum(self.s)
        if tx_bits[action]==0:
            tx_power_mW[action]=10**(self.P_MIN/10)*1e-3
        else:
            tx_power_mW[action]=10**(self.tx_powers[action]/10)*1e-3
        EE=np.zeros(self.K)
        EE[action]=(tx_bits[action]/tx_power_mW[action])/1e6
        ...
        delays=np.sum(self.e,axis=1)/np.sum(self.s>0,axis=1)
        delays[np.isnan(delays)] =0
        return np.array(self.state), EE, done,{'tx_power':tx_power_mW,'tx_bits':tx_bits,'queues_delay':delays,'queues_pkts':np.sum(self.s>0,axis=1)}
    ...
    def _recalculate_rf_old(self):
        ...
        '''Power Control'''
        rx_pwr_dbm=self.P_MAX -pathloss_db #Receiver power
    ...
    def _recalculate_rf(self):
        ...
        pathloss_db = self.propagation_model.get_free_space_pl_db(distances_m, shadowing_db=6)
        '''Power Control'''
        tx_pwr_dbm = self.P_MIN +(self.ALPHA)*pathloss_db
        tx_pwr_dbm[tx_pwr_dbm>self.P_MAX]=self.P_MAX # Transmit power
        rx_pwr_dbm=tx_pwr_dbm-pathloss_db #Receiver power
        rx_pwr_dbm_RSSI = 14 - pathloss_db  # Receiver power
        self._calculate_spectral_efficiency(rx_pwr_dbm_RSSI)
        self._spectral_efficiency_to_cqi()
        self._calculate_spectral_efficiency(rx_pwr_dbm)
        return tx_pwr_dbm
        ...
        for u, qi in enumerate(self.qi):
            if self.tti == self.tti_next_pkt[u]:
                ...
                if np.array_equal(qi, [0, 0, 0, 1]):  # 3: GBR (Conversational Voice)
                    ...
                    if buffer_gaps.size > 0:
                        self.s[u, g] = 1000
                    self.tti_next_pkt[u] = self.tti +  np.random.geometric(1 / 2/self.it)
